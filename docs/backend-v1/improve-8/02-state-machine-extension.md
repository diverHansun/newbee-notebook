...
2. **ProcessingStage 枚举化**：替代硬编码 stage 字符串
...
| 索引状态 | pgvector 和 ES 均**未建索引**，`chunk_count = 0` |
...
| COMPLETED | ✅ raw/ | ✅ markdown/ | ✅ | ✅ | ✅ |
| FAILED | ✅ raw/ | ❓ 可能部分 | ❓ 已清理 | ❓ 已清理 | ❌ |
...
claimed = await doc_repo.claim_processing(
    document_id,
    from_statuses=[UPLOADED, PENDING, FAILED],
    ...
)
...
claimed = await doc_repo.claim_processing(
    document_id,
    from_statuses=[UPLOADED, PENDING, FAILED, CONVERTED],
    ...
)
```
...
class ProcessingStage(str, Enum):
    """Document processing stage within PROCESSING status."""
    ...
    EMBEDDING    = "embedding"     # 嵌入准备（pgvector insert 内部完成）
    INDEXING_PG  = "indexing_pg"   # 写入 pgvector 向量索引
    INDEXING_ES  = "indexing_es"   # 写入 Elasticsearch 全文索引
    FINALIZING   = "finalizing"    # 更新最终元数据和状态
    COMPLETED    = "completed"     # 处理完成
    CONVERTED    = "converted"     # 转换完成（仅转换模式的终态）
...
def mark_converted(
    ...
    self.processing_stage = ProcessingStage.CONVERTED.value
...
| `_process_all_pending_async()` | 现有只查 UPLOADED/FAILED/PENDING | 根据场景决定是否包含 CONVERTED |

...
集成测试
  Slash 命令选择器：新增 /diagram 条目显示，类型不明确时确认卡片展示
...
空状态：diagrams 为空数组 → 展示引导文案（含 /diagram 提示）
...
```
chat 输入框键入 "/" → 命令选择器弹出
命令列表中包含 "/diagram"（available=true）
键入 "/d" → 列表过滤为仅 "/diagram"
点击 "/diagram" → 输入框填入 "/diagram "，光标在末尾
发送未明确类型的提示词后，收到 `confirmation_request` 并渲染确认卡片
确认后继续生成并在 done 后刷新图表列表
```

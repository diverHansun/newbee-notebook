...
Studio 首屏的"图表"卡片语义上代表所有图表类型（思维导图、流程图等），而非单一的思维导图。batch-4 仅实现 mindmap，其余类型对应的 slash 命令在选择器中显示为"即将推出"。卡片命名和内部视图结构支持未来扩展，无需重构。
...
- PNG 导出（html2canvas）
- Slash 命令选择器中新增 /mindmap 条目（复用 batch-3 组件）

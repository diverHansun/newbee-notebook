...
- `frontend/package.json` 已有 Vitest 脚本与基础测试依赖
- 仍需补齐图表依赖：`@xyflow/react`、`@dagrejs/dagre`、`html2canvas`、`mermaid`
...
| [04-slash-command.md](./04-slash-command.md) | Slash 命令选择器集成、`/diagram` 单命令触发图表生成 |
...
- **batch-3 前端**：复用 slash 命令选择器 UI 组件（新增 `/diagram` 条目）、ConfirmationCard 组件（类型确认/update/delete 图表确认）、Studio Home 卡片网格（新增"图表"卡片）

...
| 工具发现与缓存 | 是 |
| 工具适配（MCP Tool -> BaseTool） | 是 |
| 工具调用路由（BaseTool.acall -> tools/call） | 是 |
...
输入包含 `type: "http"`、`url`、`headers` 字段的 JSON。验证解析结果的 transport 为 "http"，url 和 headers 正确映射。
...
**场景：MCP 工具转为 BaseTool**

mock 一个 MCPToolInfo（name="get_weather", description="...", input_schema={...}）。调用 MCPToolAdapter.adapt()。验证返回的 BaseTool：
- name 为 `{server_name}__get_weather`。
- description 与原始描述一致。
- metadata 的参数 schema 与 input_schema 对应。

**场景：工具调用路由**

mock ClientSession.call_tool 返回包含 text content 的响应。通过适配后的 BaseTool 调用 acall(query="...")。验证 call_tool 被调用，参数正确传递，返回值为文本字符串。
...
mock call_tool 返回 isError=true 的响应。验证 BaseTool 返回错误信息字符串，不抛出异常（让 AgentLoop 的 LLM 知道工具失败后决策下一步）。
...
两个 Server 各暴露一个名为 `search` 的工具。验证适配后的 BaseTool name 分别为 `server_a__search` 和 `server_b__search`，不冲突。

...
MCP（Model Context Protocol）模块为 Agent 模式提供外部工具扩展能力。它作为 MCP Client，连接外部 MCP Server，将 Server 暴露的工具转换为 runtime 可调用的 `ToolDefinition`，与内置工具并列使用。
...
| core/tools (BuiltinToolProvider) | 内置工具与 MCP 工具并列，共享 `ToolDefinition` 协议，在 ToolRegistry 中合并 |
| core/engine | ModeConfigFactory 接收 ToolRegistry 已合并的工具列表，runtime 透明调用 |

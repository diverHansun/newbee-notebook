...
**场景：Explain 模式首轮强制后切回 auto**

config 的 tool_choice="required"。验证第一轮 LLMClient.chat() 被传入 tool_choice="required"（产出 ToolCallEvent），第二轮被传入 tool_choice="auto"，LLM 返回 finish_reason="stop"（进入流式输出）。

### 2.3 模式配置

**场景：四种模式的配置正确性**

对每种 mode 调用 ModeConfigFactory.build()，验证返回的 tools 列表、tool_choice、system_prompt 符合预期。max_iterations 统一为 50。
...
mock 工具返回包含 3 个检索结果的输出。验证 ToolCallResult.sources 包含 3 个 SourceItem。

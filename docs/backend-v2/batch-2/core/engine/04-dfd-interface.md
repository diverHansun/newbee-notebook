...
- 下游：LLMClient（llm 模块）和 tools 模块，作为 runtime 的执行依赖。
...
4. session 模块创建 runtime 实例，传入 LLMClient、ModeConfig.tools、ModeConfig.loop_policy。
5. 调用 runtime.stream(user_message, chat_history)。
6. runtime 进入循环：
...
7. runtime 产出 DoneEvent，执行结束。
8. session 模块将新消息写入 context 模块，并协调持久化。

### 2.2 最终回答的流式输出

当 runtime 检测到 LLM 不再调用工具，或质量门控允许进入 synthesis 时，需要获取 LLM 的流式文本输出。实现方式：
...
- ModeConfigFactory 将 `selected_text + optional message` 组装为 user_message。
- `LoopPolicy.require_tool_every_iteration = true`
- `LoopPolicy.required_tool_name = knowledge_base`
- 单请求内最多 3 次 retrieval iteration
- 每轮检索后都执行质量门控
- 达到门控条件或触及上限后，进入单独的 `final_synthesis`
...
**客户端关闭 SSE 连接：** API Router 检测到断开 -> 调用 runtime.cancel() -> runtime 在下一个检查点停止 -> 产出 DoneEvent（含部分结果）。

**主动取消请求：** 客户端发送 cancel 请求 -> session 模块找到活跃 runtime -> 调用 cancel() -> 同上。
...
**工具执行失败：**
```
runtime 调用工具 -> 工具抛出异常 -> runtime 构造错误 tool 消息 -> 追加到消息链 -> 继续循环（LLM 收到错误信息后自行决定）
```
...
**LLM API 错误：**
```
runtime 调用 LLMClient.chat() -> 抛出异常 -> 指数退避重试 -> 成功则继续循环 / 失败则产出 ErrorEvent
```
...
|------------|-----------|-------------|
| StartEvent | start | {"message_id": "..."} |
| WarningEvent | warning | {"code": "...", "message": "..."} |
| PhaseEvent | phase | {"stage": "reasoning"} |
| ToolCallEvent | tool_call | {"tool_name": "...", "tool_input": {...}} |
| ToolResultEvent | tool_result | {"tool_name": "...", "content_preview": "...", "source_count": N, "quality_meta": {...}} |
| SourceEvent | sources | {"sources": [...]} |
| ContentEvent | content | {"delta": "..."} |
| DoneEvent | done | {} |
| ErrorEvent | error | {"code": "...", "message": "...", "retriable": bool} |

心跳事件（heartbeat）由 API Router 层独立注入（每 15 秒），不属于 runtime 的产出。
...
|------|--------|------|
| start | start | 保留 |
| thinking | phase | thinking 仅兼容映射，不再是正式协议 |
| content | content | 不变 |
...
| mode | string | "chat" / "agent" / "ask" / "explain" / "conclude" |
| message | string, optional | Agent/Ask 必填 |
| selected_text | string, optional | Explain/Conclude 必填（通过 context 传入） |
| source_document_ids | list[string], optional | 文档范围过滤 |
| rag_config | object, optional | RAG 参数覆盖 |

校验规则：mode 为 agent/ask/chat 时 message 必填；mode 为 explain/conclude 时 `context.selected_text` 与 `context.document_id` 必填。
...
async def stream(self, message, chat_history):
    ...
    while iteration < self.config.max_iterations:
        ...
        if self._mode_requires_tool_every_iteration() and not choice.message.tool_calls:
            messages = self._append_invalid_tool_repair(messages)
            continue
        ...
        if self._should_enter_synthesis(iteration, all_quality_meta):
            break
    else:
        # max_iterations 安全熔断

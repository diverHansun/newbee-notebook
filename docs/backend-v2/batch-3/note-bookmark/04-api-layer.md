...
- 保持资源嵌套路由风格，与现有 `documents` / `notebooks` API 一致
- `mark_id` / `note_id` 是响应字段，不代表数据库物理列名
- `note_mark_refs` 由 `PATCH /api/v1/notes/{note_id}` 时后端自动同步，不单独暴露写接口
...
```
GET /api/v1/notebooks/{notebook_id}/marks?document_id=xxx
```

`document_id` 查询参数可选。响应格式同上，按 created_at 降序排列。Studio 面板书签列表使用。
...
`note_mark_refs` 不提供独立的前端写接口。约定如下：

- 前端在笔记正文中插入 `[[mark:{mark_id}]]`
- 前端保存笔记时仅调用 `PATCH /api/v1/notes/{note_id}`
- 后端 `NoteService.update_note()` 在持久化内容后解析正文中的 `[[mark:...]]`，全量同步 `note_mark_refs`
- `GET /api/v1/notes/{note_id}` 仍返回当前引用的 marks 摘要，供前端编辑器展示

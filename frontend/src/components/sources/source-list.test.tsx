...
import type { Mark, NotebookDocumentItem } from "@/lib/api/types";
...
const mockListMarksByDocument = vi.fn();
...
vi.mock("@/lib/api/marks", () => ({
  listMarksByDocument: (...args: unknown[]) => mockListMarksByDocument(...args),
}));
...
describe("SourceList", () => {
  beforeEach(() => {
    ...
    mockListMarksByDocument.mockResolvedValue({
      marks: markRows,
  ...
  it("shows a bookmark cascade warning in the remove dialog", async () => {
    ...
    expect(
      await screen.findByText(/This document has 2 bookmarks\. Removing it will remove them as well\./)
    ).toBeInTheDocument();
  });
});

...
async def test_client_manager_loads_enabled_servers_caches_tools_and_skips_disabled(tmp_path: Path):
        ...
        return _FakeClient(
            ...
            [
                MCPToolInfo(
                    ...
                    name="search",
                    qualified_name=f"{config.name}__search",
    ...
    assert [tool.name for tool in tools] == ["weather__search"]
    assert [tool.name for tool in cached] == ["weather__search"]
    ...
    assert [tool.name for tool in again] == ["weather__search"]
    assert connect_calls == ["weather"]
...
async def test_client_manager_degrades_failed_servers_without_blocking_other_tools(tmp_path: Path):
        ...
        return _FakeClient(
            ...
            [
                MCPToolInfo(
                    server_name=config.name,
                    name="forecast",
                    qualified_name=f"{config.name}__forecast",
    ...
    assert [tool.name for tool in tools] == ["weather__forecast"]
